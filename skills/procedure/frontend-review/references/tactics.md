...
- **[T] トークン化済み** — `ui-design` が確定したトークン CSS で既に強制済み。実装中に能動的な self-check は不要
- **[S] 画面ごと判断** — 実装中に能動的に self-check して適用する

実装に入ったら **[S] tactic のみ**を能動的に self-check する。

## CSS 例の書式

CSS 例は **raw 値を `:root` (token 定義層) に置き、使用側は `var(--*)` で参照する** 2 層で書いてある。静的検査の F1 (8pt scale) / F3 (生 hex 直書き禁止) は `:root` 以外の raw 値を Red Flag にするため、この形で写せば合格条件 (Red Flag = 0) と衝突しない。

`:root` を併記した例の token は「どんな値を意図しているか」を見せるための抜粋で、実物は `ui-design` が書いた `tokens.md` が正本 (骨格 CSS は `skills/design/ui-design/references/scale-templates/` にあり、`--space-*` は `spacing-8pt.css`、`--shadow-*` は `layout-12col.css` 由来)。
...
- **段階の値:** `ui-design` の `tokens.md` が正本 (骨格は `skills/design/ui-design/references/scale-templates/type-scale-{1.20,1.25,1.333}.css`)。ここに値を再掲しない (再掲すると同じ token 名が 2 つの値を持つ)。実装側は `font-size: var(--text-lg)` の形で参照する
