...
description: swat-skills の正本 settings (permission / sandbox) の原則を cwd の project へ適用するインストーラ。追加 / 変更 (理由付き) / 保留 の 3 分類で提案し、人間が承認した分だけ書く。
---
...
| 適用先の transcript | **読まない**。本 skill の証拠は正本と本文の原則だけで、実績から減らす / 直す側ではない |
...
4. 書き先を決める。**既定は `.claude/settings.local.json`** で、そのまま進む。起動時の依頼文に共有意図 (「チームで」「commit したい」「CI でも効かせたい」「fresh worktree でも効かせたい」) が読めるときだけ、`.claude/settings.json` にするかを尋ねる。既定側は Claude Code の既定で gitignored なので、fresh worktree / fresh clone には存在せず、そこで動くセッションには適用されない — 共有意図はこの形でも現れる。一方、共有側は個人スコープの許可を repo の共有設定として他者へ配ることになるので、既定にはしない
...
- `~/.claude/settings.json` — 衝突検知のためだけに読む。permission は **`deny` > `ask` > `allow`** の順で、かつ上位 scope が勝つ形で評価される。global の deny / ask に当たる entry は project 側へ allow を置いても一度も許可として機能しない
...
適用した entry のうち 1 つを実際に走らせ、permission ask が消えたことを確かめる。消えないときは、entry より外側の層まで順に辿る。

- **上位 scope** — global の deny / ask が当たっていないか (評価順は手順 2)
- **auto-allow の例外** — `autoAllowBashIfSandboxed: true` でも素通りしないものが 3 つある: ① 明示的な `permissions.deny`、② **content-scoped な `permissions.ask` entry (`Bash(rm:*)` 等) は sandbox 内でもプロンプトを強制する**、③ `/`・home・重要 system path を対象にした `rm` / `rmdir`。ask を丸ごとスキップできるのは裸の `Bash` / `Bash(*)` ask のみ
- **hook 層** — PreToolUse hook の `allow` は permission rule を越えない。同じコマンドに match する `ask` がある間、許可専用 hook はプロンプトを消せない (逆方向の hook deny / exit 2 は permission rule より先に効く)
- **entry の綴り** — 照合はコマンドの綴りと token 境界に依存する (「entry を足す / 足さない基準」の deny 項)
- **`:*` の有無** — path 中に `*` を置いた entry が死んでいないか (「path 解決」節)
...
| issue tracker | `Bash(gh issue view:*)` `Bash(gh issue list:*)` `Bash(gh issue comment:*)` `Bash(gh issue edit:*)` `Bash(gh pr view:*)` `Bash(gh pr list:*)` `Bash(gh pr diff:*)` `Bash(gh pr checks:*)` `Bash(gh pr create:*)` `Bash(gh pr comment:*)` `Bash(gh repo view:*)` `Bash(gh api:*)` (GitLab の project なら `glab` の対応 entry) | tracker は project ごとに違うので、使う側だけ入れる。**issue の state 変更 (`gh issue close` / `glab issue close`) を足すのは、issue 置き場が gh / glab で PR がそれと別 repo に出る project だけ** — closing reference が届かず、merge しても issue が open のまま残るため。同 repo なら PR merge の auto-close で足りるので入れない (issue 置き場が Jira の project も CLI ではなく Rovo MCP を使うので不要)。**issue の起票 (`gh issue create` / `glab issue create`) を足すのは、issue 置き場が gh / glab で dispatch 機構を入れる project だけ** — orchestrator が user 直依頼をその場で起票する経路でだけ使う (jira 置き場は close と同じく Rovo MCP なので不要) |
| read-only 調査 | `Bash(ls:*)` `Bash(grep:*)` `Bash(rg:*)` `Bash(wc:*)` `Bash(jq:*)` `Bash(yq:*)` `Bash(cat:*)` `Bash(head:*)` `Bash(tail:*)` `Bash(sort:*)` `Bash(uniq:*)` `Bash(comm:*)` `Bash(diff:*)` `Bash(test:*)` `Bash(echo:*)` `Bash(sleep:*)` `Bash(mkdir:*)` `Bash(chmod +x:*)` `Bash(pwd)` `Bash(date:*)` `Bash(tree:*)` | 自律的なコード理解を妨げないための最小セット。ファイル探索の正規手段は `rg --files` / `ls` / Glob tool で、**`find` は入れない** — 適用先の global に `Bash(find:*)` deny があれば allow は機能せず (手順 2 の評価順)、無くても同じ探索が通る。`find` 固有の危険は列挙ではなく `-exec` / `-delete` 系の述語なので、塞ぐなら allow / deny ではなく hook 側で述語だけを見る |
...
| sandbox 脱出 (普遍) | `excludedCommands` に `gh:*` / `glab:*` / `herdr:*` と、subprocess で gh・herdr を起動する skill script の tilde path | **sandbox の設計由来**の制約。`gh` / `glab` は credential (`~/.config/gh` 等) が sandbox の credential 保護で読めず起動失敗、`herdr` は socket connect が遮断される。script 経由の起動も同じ理由で除外が要る。**除外したコマンドには sandbox の制約が一切かからない** ので、守りは permission 層だけになる — 破壊的な subcommand を含むものは deny と対で扱う |
| filesystem 書込 | `allowWrite` に `~/.claude/metrics` / `~/.cache/uv` / `~/.claude/issue-dispatch` / `~/.claude/dispatch-v2` | 順に permission 記録・uv 実行の cache・同梱 MCP server の台帳置き場 2 つ (dispatch v1 / v2 は別台帳で併走する)。いずれも cwd 外なので既定では書けない。**台帳置き場は daemon 自身の書き込みには要らない** (daemon は Bash sandbox の外) — 宣言 config `dispatch-project.toml` をセッションから設置する経路に要る |
...
**deny は綴りと token 境界で照合される** ので、破壊的操作の防御は deny 単独で完結させず、sandbox の OS 境界と hook 層を重ねる。`Bash(rm:*)` が当たるのは先頭 token が `rm` のときだけで、`command rm` は wrapper stripping (`command` / `builtin` / `timeout` / `nice` / `nohup` / `stdbuf` / `noglob` / フラグなし `xargs`) の対象なので当たるが、**`/bin/rm` のような path 接頭辞つきの綴りは当たらず無言で通過する**。prefix 照合も token 境界を見る (一致条件が「完全一致」か「直後が空白」) ため、`Bash(git push --force:*)` は `git push --force-with-lease` に当たらない。
