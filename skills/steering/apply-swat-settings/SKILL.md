...
| git (書き込み / 統合) | `Bash(git add:*)` `Bash(git commit:*)` `Bash(git restore:*)` `Bash(git switch:*)` `Bash(git checkout -b:*)` `Bash(git worktree:*)` `Bash(git rebase:*)` `Bash(git merge:*)` `Bash(git push:*)` `Bash(git pull:*)` `Bash(git fetch:*)` `Bash(git ls-remote:*)` | issue 駆動フロー (実装 → commit → PR) で毎回出る |
| issue tracker | `Bash(gh issue view:*)` `Bash(gh issue list:*)` `Bash(gh issue comment:*)` `Bash(gh issue edit:*)` `Bash(gh issue close:*)` `Bash(gh pr view:*)` `Bash(gh pr list:*)` `Bash(gh pr diff:*)` `Bash(gh pr checks:*)` `Bash(gh pr create:*)` `Bash(gh pr comment:*)` `Bash(gh repo view:*)` `Bash(gh api:*)` (GitLab の project なら `glab` の対応 entry) | tracker は project ごとに違うので、使う側だけ入れる |
...
9. **ファイル探索の正規手段は `rg --files` / `ls` / Glob tool**。`find` の allow を足さない (落とし穴 1 の実例。deny されても `rg --files` / `ls` へ切り替えれば同じ探索が通る)
