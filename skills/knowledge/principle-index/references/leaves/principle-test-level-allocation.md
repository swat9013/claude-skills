...
- **受け入れ条件・ユースケースの振る舞いを検査する自動テスト (e2e・統合テスト) を最優先で書く**。ユースケースは受け入れ条件であり、自動テストに落とす (`principle-design-derivation-order` の進め方と同一原則)
...
既存テストを残すか削るかの判断は `principle-test-pruning` が扱う。
