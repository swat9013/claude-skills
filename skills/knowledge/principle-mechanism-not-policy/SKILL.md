...
- **判定は「その記述が実行 project の規約と衝突しうるか」**。衝突しうるなら policy なので委ねる。運用そのものを主題にする配布物 (作業ツリーや PR を扱う skill 等) と、動作環境の事実の記述はこの線の外
- **frontmatter の `description` も本文と同じ線で見る**。手順から運用指示を外しても description に残れば、運用方針は配られたまま
