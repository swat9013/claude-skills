...
reference と index の書き込み先は plugin の上流 repo `swat9013/swat-skills` の `skills/knowledge/dev-env-best-practices/`。この skill は references/ ごと swat-skills plugin 同梱で配布されるので、上流の repo を編集した分だけが次の更新後も残る。
