...
- **土台は常に主幹の最新にする**: 着手時に最新を取り込み、安定版を狙って取り込む時期をずらさない。最新で問題が出たらその場で直す (見送って古い土台に留まらない)。fetch / pull のコストは払う前提で組む。※短命ブランチが「作った差分をどれだけ短命に保つか」なのに対し、本規則は「土台をどれだけ新しく保つか」。
...
- **自律実行の制御は選択肢を広げる方向で設計する**。禁止を積み増して制御を強めるのではなく、破壊的・不可逆な副作用を持つ操作だけを塞ぎ、リスクのない操作は許可して委譲範囲を広げる。禁止は操作の種類ではなく副作用の有無で切る。拒否が頻出する操作は危険性ではなく設計の摩擦の指標 — 拒否で突き返して代替探索の往復を増やすのは、制御が効いているのではなく設計が失敗している状態。
