...
### 取り込みの鮮度 (2026-08-22 追加)

**価値**: 開発は常に主幹の最新を土台にする。着手時に最新を取り込み、安定版を狙って取り込む時期をずらさない。最新のコードで問題が出たらそのまま直す — 取り込みを見送って回避しない。取り込みのための fetch / pull コストは払う前提で組む。Q11 が「作った差分をどれだけ短命に保つか」を扱うのに対し、本項は**土台をどれだけ新しく保つか**を扱う (競合しない)。

**背景**: 主幹の更新を「外部ライブラリのアップデート」と同じ扱いにして安定版を待つと、自分たちが書いたコードの統合を先送りすることになり、後から出る衝突と手戻りが大きくなる。自律実行の起動文でも、複数 repo に共通して「着手前に必ず主幹の最新を取り込む」が作業ルールの先頭に置かれている。取り込み後に不整合が出た場合の既定は「見送って古い土台に戻る」ではなく「その場で直す」。
...
### 自律実行のガードレール設計 (2026-08-22 追加)

**価値**: 自律実行の制御は、禁止を積み増して制御を強める方向ではなく、**安全に使える選択肢を広げる**方向で設計する。危険な操作 (破壊的・不可逆な副作用を持つもの) だけを塞ぎ、リスクのない操作は許可して委譲範囲を広げる。拒否で突き返して代替コマンドを探させ往復を増やすのは、制御が効いているのではなく設計が失敗している状態。禁止対象は操作の**種類**ではなく副作用の有無で切る。「委譲境界と引き渡し」(2026-08-03) が任せた作業をどこで切り上げるかを扱うのに対し、本項は**委譲できる範囲をどう広げるか**を扱う (競合しない)。

**背景**: エージェントの権限設定を見直す場面で、拒否が頻出する操作ほど「エージェントが自然に選ぶ操作」であるという観測が起点になっている。頻出する拒否は、危険性の指標ではなく**設計の摩擦**の指標として読む。同じ命令でも破壊的オプションを伴うものだけを塞ぎ、純粋な読み取り・列挙は許可する、という粒度まで下げられるならそれを選ぶ。塞いだままにすると、代替手段の探索と再実行で往復が増え、実行コストだけが上がって安全性は変わらない。
